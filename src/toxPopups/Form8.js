...
const Form8 = ({ open, setOpen, data, getFormData }) => {
    ...
    function addRecord() {
        ...
        setLoading(true)
        axios.post(`${baseUrl}toxsection8data`, obj)
            .then((resp) => {
    ...
    function edit() {
        setLoading(true)
        axios.put(`${baseUrl}toxsection8data/${formData?.row_id}`, formData)
            .then((resp) => {
    ...
    return (
        <Modal centered className='form-30' open={open} style={{ width: '35%' }} onCancel={() => setOpen(false)} title={formData?.row_id ? 'Edit Record' : 'Add New Record'} footer={[
            ...
            <div>
                <p>Date:</p>
                <DatePicker className='w-100 mb-3' onChange={handleDate} value={formData?.Date && dayjs(formData?.Date)}/>
            </div>
            <div>
                <p>Comments:</p>
